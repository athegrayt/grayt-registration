    ...
    form.parse(req, (err, fields, files) => {
        ...
        if (files.photo) {
            if (files.photo.size > 1000000) {
                return res.status(400).json({
                    error: "Please make sure that image size is less than 1mb"
                })
            }
...
exports.remove = (req, res) => {
    ...
    product.deleteOne((err, productRemoved) => {
        if (err) {
            return res.status(400).json({
                error: errorHandler(err)
            })
...
exports.update = (req, res) => {
    ...
    form.parse(req, (err, fields, files) => {
        ...
        let product = Object.assign(req.product, fields)
...
//Lists the categories that are being used by the products
