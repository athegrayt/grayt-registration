...
describe("Category Routes", () => {
    test("GET /category/:categoryId", async () => {
        //arrange
            ...
            //Create Category
        const res = await request.post(`/api/category/create/${userInfo[1]}`).auth(`${userInfo[0]}`, { type: 'bearer' }).type('application/json').send({
    ...
    test("Error if category already exists (Create)", async (done) => {
        ...
        try {
            //act
            const category1 = await request.post(`/api/category/create/${userInfo[1]}`).auth(`${userInfo[0]}`, { type: 'bearer' }).type('application/json').send({
                name: `${testCategories[0]}`
            })
            const res = await request.post(`/api/category/create/${userInfo[1]}`).auth(`${userInfo[0]}`, { type: 'bearer' }).type('application/json').send({
                name: `${testCategories[0]}`
            ...
            expect(res.body.error).toBe(`"${testCategories[0]}" already exists as a category`)
            done()
        } catch (err) {
            console.log(err);
            done(err)
    ...
    test("DELETE /category/:categoryId/:userId", async() => {
        //arrange
            //Create Product
        const category = await request.post(`/api/category/create/${userInfo[1]}`).auth(`${userInfo[0]}`, { type: 'bearer' }).type('application/json').send({
                name: `${testCategories[2]}`
            })
        //act
            //Delete Product 
        const res = await request.delete(`/api/category/${category.body._id}/${userInfo[1]}`).auth(`${userInfo[0]}`, { type: 'bearer' })
        //assertion
        expect(res.status).toBe(200)
    ...
    test("PUT /category/:categoryId/:userId (Update)", async (done) => {
        ...
        try {
            ...
            const category = await request.post(`/api/category/create/${userInfo[1]}`).auth(`${userInfo[0]}`, { type: 'bearer' }).type('application/json').send({
                ...
                //Update Product
            const res = await request.put(`/api/category/${category.body._id}/${userInfo[1]}`).auth(`${userInfo[0]}`, { type: 'bearer' }).send({
