...
describe("GET /secret/:userId (Admin Access)", () => {
    test("failing to access user with Id due to not being admin", async (done) => {
        //arrange
        try {
            const userInfo = await request.post('/api/signin').send({
            ...
            //act
            const res = await request.get(`/api/secret/${userInfo[1]}`).auth(`${userInfo[0]}`, { type: 'bearer' }).type('application/json')
            //assertions
    ...
    test("get user data", async (done) => {
        try {
            //act
            const res = await request.get(`/api/user/${userInfo[1]}`).auth(`${userInfo[0]}`, { type: 'bearer' }).type('application/json')
            //assertions
            expect(res.status).toBe(200)
    ...
    test("update user data", async (done) => {
        try {
            //act
            const res = await request.put(`/api/user/${userInfo[1]}`).auth(`${userInfo[0]}`, { type: 'bearer' }).type('application/json').send({
                firstName: 'BERRY'
            })
            //assertions
            expect(res.status).toBe(200)
            expect(res.body.firstName).toBe('BERRY')
            const result = await request.put(`/api/user/${userInfo[1]}`).auth(`${userInfo[0]}`, { type: 'bearer' }).type('application/json').send({
                firstName: 'Berry'
            })
            //assertions
